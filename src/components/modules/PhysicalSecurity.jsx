...
const PhysicalSecurity = () => {
  ...
  const handleRegisterVisitor = () => {
    toast.success("Opening visitor registration terminal...");
  };
      ...
      <div className="py-8 flex-1">
        ...
        <div className="mb-8 flex flex-col md:flex-row md:items-end md:justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500 font-inter">
              Live monitoring, visitor management, and physical perimeter control.
            </p>
          </div>
          <div className="flex items-center gap-4 bg-white px-4 py-2 rounded-lg border border-gray-200 shadow-sm">
            ...
            <div className="flex items-center gap-2 border-l border-r border-gray-200 px-4">
              ...
              <span className="font-mono font-medium text-gray-800">{currentTime}</span>
            </div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-gray-600">System Normal</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          ...
          >
            ...
            <span className="text-sm font-semibold text-gray-800">New Visitor</span>
          </button>
          ...
          <button className="flex flex-col items-center justify-center p-4 bg-white rounded-xl border border-gray-200 hover:border-purple-400 hover:shadow-md transition-all group">
            ...
            <span className="text-sm font-semibold text-gray-800">Issue Badge</span>
          </button>
          ...
          <button className="flex flex-col items-center justify-center p-4 bg-white rounded-xl border border-gray-200 hover:border-emerald-400 hover:shadow-md transition-all group">
            ...
            <span className="text-sm font-semibold text-gray-800">Dispatch Guard</span>
          </button>
          ...
          >
            ...
            <span className="text-sm font-semibold text-red-600">Trigger Alarm</span>
          </button>
        </div>
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
              ...
              <div className="p-4 bg-neutral-900 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 min-h-[400px]">
                {isLoading ? (
                  ...
                  (cameras || []).map((camera) => (
                    <div key={camera._id} className="relative group rounded-lg overflow-hidden bg-neutral-800 aspect-video border border-neutral-700">
                      {/* Mock Camera Feed Background */}
                      <div className={`absolute inset-0 opacity-20 ${camera.status === 'online' ? 'bg-blue-400' : 'bg-red-900'} mix-blend-overlay`}></div>
                      ...
                      {camera.status === 'online' ? (
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center text-neutral-500">
                          ...
                          <span className="text-xs font-mono uppercase tracking-wider">No Signal</span>
                        </div>
                      )}
                      ...
                      <div className="absolute top-0 left-0 w-full p-2 flex justify-between items-start bg-gradient-to-b from-black/60 to-transparent">
                        <div className="flex items-center gap-2">
                          <span className={`h-2 w-2 rounded-full ${camera.status === 'online' ? 'bg-red-500 animate-pulse' : 'bg-neutral-600'}`}></span>
                          <span className="text-white text-xs font-mono font-medium drop-shadow-md">CAM-{camera._id.substring(camera._id.length - 4).toUpperCase()}</span>
                        </div>
                      ...
                      <div className="absolute bottom-0 left-0 w-full p-2 bg-gradient-to-t from-black/80 to-transparent flex items-end justify-between">
                        ...
                        {camera.status === 'online' && camera.lastMotion && (
                          <div className="flex flex-col items-end">
                             <span className="text-[10px] text-blue-300 font-mono uppercase">Last Motion:</span>
                             <span className="text-xs font-mono text-white">{camera.lastMotion}</span>
                          </div>
                        )}
                      </div>
                    </div>
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
              ...
              <div className="p-4 space-y-4">
                {isLoading ? (
                  ...
                  <p className="text-sm text-gray-500 text-center py-4">No personnel currently listed on duty.</p>
                ) : (
                  (personnel || []).map((p) => (
                    <div key={p._id} className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <img src={p.avatarUrl || `https://ui-avatars.com/api/?name=${encodeURIComponent(p.name)}`} className="w-10 h-10 rounded-full" alt="Guard" />
                        <div>
                          <p className="text-sm font-semibold text-gray-900">{p.name}</p>
                          <p className="text-xs text-gray-500">{p.role} {p.shift ? `• ${p.shift}` : ''}</p>
                        </div>
                      </div>
                      <button className="text-blue-600 hover:bg-blue-50 p-2 rounded-full transition-colors" title="Contact">
                      ...
                      <li key={log._id} className="p-4 hover:bg-gray-50 transition-colors">
                        <div className="flex justify-between items-start mb-1">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-semibold uppercase tracking-wider text-gray-500">{log.type}</span>
                          </div>
