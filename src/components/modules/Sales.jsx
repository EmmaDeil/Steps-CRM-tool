...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  <div>
  <div style="margin-top:62px;padding:16px;background:#f1f5f9;border-radius:8px;font-size:12px;color:#374151">
  </div>
...
const Sales = () => {
  ...
  const fetchDeals = useCallback(async () => {
    try {
      ...
      if (res) {
        setDeals(res.deals || []);
        setStats(res.stats || { totalDeals: 0, wonValue: 0, pipelineValue: 0, conversionRate: 0, activeCount: 0 });
      }
  ...
  const filteredDeals = useMemo(() => {
    ...
    return (deals || []).filter(d => {
      ...
      const match = title.includes(q) || customerName.includes(q);
      ...
      <div className="p-4 md:p-6 space-y-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <button onClick={() => setShowDealModal(true)}
            className="inline-flex items-center gap-2 px-4 py-2.5 border border-gray-300 bg-blue-500 text-white rounded-lg font-semibold shadow-sm transition-colors text-sm hover:bg-blue-600 self-start sm:self-auto">
