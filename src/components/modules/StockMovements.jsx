...
const StockMovements = () => {
  ...
  const fetchMovements = useCallback(async (pageNum = 1) => {
    try {
      setLoading(true);
      const res = await apiService.get(`/api/inventory-movements?page=${pageNum}&limit=50`);
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200">
      <div className="p-4 border-b border-gray-200">
        ...
        <p className="text-xs text-gray-500 mt-1">Audit trail of all inventory changes</p>
      </div>
                ...
                return (
                  <tr key={m._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 font-medium text-gray-800">
                      {m.inventoryItemId?.name || <span className="text-gray-400 italic">Unknown Item</span>}
                    </td>
                    ...
                    <td className={`px-4 py-3 text-right font-bold ${isPositive ? 'text-green-600' : m.quantityChange < 0 ? 'text-red-600' : 'text-gray-400'}`}>
      ...
      {totalPages > 1 && (
        <div className="p-3 border-t border-gray-200 flex items-center justify-between text-sm text-gray-500 bg-gray-50">
          <div className="flex gap-1 ml-auto">
            <button onClick={() => fetchMovements(page - 1)} disabled={page <= 1} className="px-3 py-1 border rounded disabled:opacity-50">Prev</button>
            <button onClick={() => fetchMovements(page + 1)} disabled={page >= totalPages} className="px-3 py-1 border rounded disabled:opacity-50">Next</button>
          </div>
        </div>
      )}
    </div>
