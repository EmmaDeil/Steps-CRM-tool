...
const GenerateInvoiceModal = ({ issue, onClose, onGenerated }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await apiService.post(
        `/api/inventory-issues/${issue._id}/generate-invoice`,
        form,
      );
  ...
  const subtotal =
    issue.lineItems?.reduce((s, li) => s + (li.totalPrice || 0), 0) || 0;
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-sm">
            <p className="font-semibold text-blue-800">
              {issue.issueNumber} — {issue.issuedTo}
            </p>
            <p className="text-blue-600 mt-1">
              {issue.lineItems?.length} item(s) · Base value:{" "}
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none text-sm"
            >
              {["Net 7", "Net 14", "Net 30", "Net 60", "Due on Receipt"].map(
                (t) => (
          ...
          {form.taxRate > 0 && (
            <div className="bg-gray-50 rounded-lg p-3 text-xs text-gray-600 flex justify-between">
              ...
              <span>
                Tax ({form.taxRate}%): {formatCurrency(tax)}
              </span>
...
const Invoicing = ({ onBackToFinance }) => {
  ...
  const fetchIssues = useCallback(
    async (page = 1, search = "", status = "all") => {
      try {
        ...
        toast.error("Failed to load inventory issues");
      } finally {
    ...
    async (page = 1, search = "", status = "all") => {
      try {
        ...
        if (status === "all" || status === "payable" || status === "paid") {
          const poRes = await apiService.get(
          ...
          const paidPoRes = await apiService.get(
            "/api/purchase-orders?status=paid&limit=50",
          );
          ...
          const uniquePos = Array.from(
            new Map(pos.map((po) => [po._id, po])).values(),
          );

          // Filter by search if needed locally
          const filteredPos = uniquePos
            .filter((po) => {
  ...
  useEffect(() => {
    const t = setTimeout(
      () => fetchIssues(1, issueSearch, issueStatusFilter),
      400,
    );
    ...
    return (
      <div className="w-full min-h-screen bg-gray-50">
        ...
        <InvoiceDetail
          ...
          onBack={() => setSelectedInvoiceView(null)}
        />
      </div>
    );
  }
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center my-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            Manage stock issues and generate invoices
          </p>
        </div>
      </div>
        ...
        <nav className="-mb-px flex space-x-8">
          {[
            ...
            >
              ...
              {tab.count > 0 && (
                <span className="bg-gray-100 text-gray-600 rounded-full px-2 py-0.5 text-xs font-bold">
                  {tab.count}
                </span>
              )}
            </button>
            ...
            {issuesLoading ? (
              ...
              <div className="p-10 text-center">
                ...
                <p className="text-sm text-gray-400 mt-1">
                  Issues appear here when stock is issued from the Inventory
                  module
                </p>
              </div>
            ) : (
                ...
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    {[
                      ...
                      <th
                        key={h}
                        className="px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide text-left"
                      >
                        {h}
                      </th>
                    ))}
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          <button
                            onClick={() =>
                              setSelectedIssue(
                                selectedIssue?._id === issue._id ? null : issue,
                              )
                            }
            ...
            <div className="border-t border-gray-200 bg-blue-50/50 p-4">
              <div className="flex justify-between items-start mb-3">
                <h4 className="font-bold text-gray-800">
                  {selectedIssue.issueNumber} — {selectedIssue.issuedTo}
                </h4>
                <button
              ...
              <table className="w-full text-xs border-collapse">
                ...
                <tbody>
                  {selectedIssue.lineItems?.map((li, i) => (
                ...
                <tfoot>
                  <tr>
                    ...
                    <td className="p-2 text-right font-bold text-emerald-700">
                      {formatCurrency(selectedIssue.totalValue)}
                    </td>
                  </tr>
                </tfoot>
              </table>
              {selectedIssue.status === "issued" && (
                <div className="mt-3 flex justify-end">
                  ...
                  >
                    <i className="fa-solid fa-file-invoice-dollar"></i> Generate
                    Invoice
                  </button>
                </div>
              )}
          ...
          {issuesTotalPages > 1 && (
            <div className="p-3 border-t border-gray-200 flex items-center justify-between text-sm text-gray-500 bg-gray-50">
              ...
              <div className="flex gap-1">
                <button
                  onClick={() =>
                    fetchIssues(issuesPage - 1, issueSearch, issueStatusFilter)
                  }
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
          ...
          <div className="p-4 border-b border-gray-200 flex flex-col sm:flex-row gap-3 items-center">
            <div className="relative w-full sm:w-72">
              ...
              <input
                type="text"
                placeholder="Search invoice number or bill to…"
                value={invoiceSearch}
          ...
          <div className="overflow-x-auto">
            {invoicesLoading ? (
              ...
              <div className="p-10 text-center">
                ...
                <p className="text-sm text-gray-400 mt-1">
                  Generate an invoice from a Stock Issue to see it here
                </p>
              </div>
            ) : (
              <table className="w-full text-sm">
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          ...
                          {!inv._isAP && (
                            <button
                              onClick={() => handlePrintInvoice(inv._id)}
                              className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded transition-colors"
                              title="Print invoice"
                            >
                          ...
                          {inv.status === "sent" && (
                            <button
                              onClick={() =>
                                handleStatusChange(inv._id, "paid")
                              }
            ...
            <div className="p-3 border-t border-gray-200 flex items-center justify-between text-sm text-gray-500 bg-gray-50">
              ...
              <div className="flex gap-1">
                ...
                <button
                  ...
                  disabled={
                    invoicesPage >= invoicesTotalPages || invoicesLoading
                  }
                  className="px-3 py-1 border rounded text-xs disabled:opacity-40"
                >
                  Next
