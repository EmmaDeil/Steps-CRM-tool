...
router.post('/pos/:id/record-payment', async (req, res) => {
  try {
    const { id } = req.params;
    const { amount, paymentType, paymentMethod = 'bank_transfer', reference = {} } = req.body;
...
router.post('/material-requests/:id/generate-rfq', async (req, res) => {
  try {
    ...
    await AuditLogModel.create({
      ...
      description: `RFQ ${rfq.rfqNumber} generated from Material Request`,
      ipAddress: req.ip,
      status: 'Success',
    });
...
router.post('/rfqs/:id/generate-po', async (req, res) => {
  try {
    ...
    await AuditLogModel.create({
      ...
      description: `PO ${po.poNumber} created from RFQ`,
      status: 'Success',
    });
...
router.post('/pos/:id/record-payment', async (req, res) => {
  try {
    ...
    await AuditLogModel.create({
      ...
      description: `Payment of ${amount} NGN recorded for PO ${result.updatedPO.poNumber}`,
      status: 'Success',
    });
...
router.post('/pos/:id/receive-items', async (req, res) => {
  try {
    ...
    await AuditLogModel.create({
      ...
      description: `${receivedItems.length} items received for PO ${result.updatedPO.poNumber}. Receipt: ${result.receipt.receiptNumber}`,
      status: 'Success',
    });
