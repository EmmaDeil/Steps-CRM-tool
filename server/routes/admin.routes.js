...
const generateAppApiKey = () => `app_${crypto.randomBytes(24).toString('hex')}`;
    ...
    try {
        ...
        if (!settings) {
            settings = await SystemSettings.create({
                appApiKey: generateAppApiKey(),
                appApiKeyGeneratedAt: new Date(),
            });
        } else if (!String(settings.appApiKey || '').trim()) {
            settings.appApiKey = String(settings.attendanceApiKey || '').trim() || generateAppApiKey();
            settings.appApiKeyGeneratedAt = new Date();
            await settings.save();
        }
        res.json(settings);
    } catch (error) {
...
// ============================================================
// POST /api/admin/system-settings/app-api-key/generate
// Generate and persist app API key
// ============================================================
router.post('/system-settings/app-api-key/generate', async (req, res) => {
    try {
        let settings = await SystemSettings.findOne();
        if (!settings) {
        ...
        settings.appApiKey = generateAppApiKey();
        settings.appApiKeyGeneratedAt = new Date();
        await settings.save();
        ...
        await AuditLog.create({
            ...
            ipAddress: req.ip || req.connection?.remoteAddress || '127.0.0.1',
            userAgent: req.headers['user-agent'],
                description: `Application API key generated by ${req.user.email}`,
            status: 'Success',
        });

        res.json({
                message: 'Application API key generated successfully',
                appApiKey: settings.appApiKey,
                appApiKeyGeneratedAt: settings.appApiKeyGeneratedAt,
                appApiKeyLastUsedAt: settings.appApiKeyLastUsedAt,
        });
    } catch (error) {
            console.error('Error generating application API key:', error);
            res.status(500).json({ message: 'Failed to generate application API key' });
    }
});
