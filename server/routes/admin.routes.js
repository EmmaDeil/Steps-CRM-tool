...
const generateAttendanceApiKey = () => `attn_${crypto.randomBytes(24).toString('hex')}`;
    ...
    try {
        ...
        if (!settings) {
            settings = await SystemSettings.create({
                attendanceApiKey: generateAttendanceApiKey(),
                attendanceApiKeyGeneratedAt: new Date(),
            });
        } else if (!String(settings.attendanceApiKey || '').trim()) {
            settings.attendanceApiKey = generateAttendanceApiKey();
            settings.attendanceApiKeyGeneratedAt = new Date();
            await settings.save();
        }
        res.json(settings);
    } catch (error) {
...
// ============================================================
// POST /api/admin/system-settings/attendance-api-key/generate
// Generate and persist attendance API key
// ============================================================
router.post('/system-settings/attendance-api-key/generate', async (req, res) => {
    try {
        let settings = await SystemSettings.findOne();
        if (!settings) {
        ...
        await AuditLog.create({
            ...
            ipAddress: req.ip || req.connection?.remoteAddress || '127.0.0.1',
            userAgent: req.headers['user-agent'],
            description: `Attendance API key generated by ${req.user.email}`,
            status: 'Success',
        });

        res.json({
            message: 'Attendance API key generated successfully',
            attendanceApiKey: settings.attendanceApiKey,
            attendanceApiKeyGeneratedAt: settings.attendanceApiKeyGeneratedAt,
            attendanceApiKeyLastUsedAt: settings.attendanceApiKeyLastUsedAt,
        });
    } catch (error) {
        console.error('Error generating attendance API key:', error);
        res.status(500).json({ message: 'Failed to generate attendance API key' });
    }
});
